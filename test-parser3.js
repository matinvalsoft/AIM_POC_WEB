...
async function testParser3() {
  try {
    console.log('🧪 Testing parser3 API...\n');
    console.log('⚠️  NOTE: This test requires a valid File record ID in Airtable');
    console.log('    Replace "YOUR_FILE_RECORD_ID" with an actual File record ID\n');
    
    // You need to replace this with an actual File record ID from your Airtable
    const fileRecordId = 'YOUR_FILE_RECORD_ID'; // e.g., 'recXXXXXXXXXXXXXX'
    
    if (fileRecordId === 'YOUR_FILE_RECORD_ID') {
      console.log('❌ Please update the test script with a valid File record ID');
      console.log('\nTo test this API:');
      console.log('1. Create a File record in Airtable (or use an existing one)');
      console.log('2. Replace "YOUR_FILE_RECORD_ID" in test-parser3.js with the actual record ID');
      console.log('3. Run this test again\n');
      console.log('Example API call:');
      console.log(`
POST http://localhost:3000/api/parser3
